...
function pintarCategoria(respuesta){
    ...
    for(i=0;i<respuesta.length;i++){
        ...
        myTable+="<td> <button class='flex mx-auto text-white bg-gray-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-500 rounded text-lg' onclick='putCategoria("+respuesta[i].id+") '> Actualizar</button>"
        myTable+="<td> <button class='flex mx-auto text-white bg-gray-800 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-500 rounded text-lg' onclick='deleteCategoria("+respuesta[i].id+")'> Borrar</button>"
        myTable+="</tr>";
    }
    myTable+="</table>";
